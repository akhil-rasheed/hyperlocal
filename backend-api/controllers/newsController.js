    ...
    try{
        ...
        if(!req.body.latitude|| !req.body.longitude){
            ...
            if(!req.body.title){
                res.status(401).send({
                    success:false,
                    message:'requires title for the news'
                });
            }
        } else {
...
export const upvotesNews = async(req,res)=>{
    try{
        ...
        if(!news.downvotes.includes(req.body.userId)){
            ...
            res.status(200).json("The post has been upvoted and removed from downvotes");
        }else {
           await news.updateOne(
            {
                $inc:{upvotescount:1},
...
export const downvotesNews = async(req,res)=>{
    try{
        ...
        if(!news.upvotes.includes(req.body.userId)){
            ...
            res.status(200).json("The post has been downvoted and removed from upvotes");
        }else {
            await news.updateOne({$pull:{downvotes:req.body.userId},$inc:{upcount:-1}});
